...
use App\Http\Requests\User\User\StoreUserequest;
use App\Models\User;
...
class UserController extends Controller
{
    ...
    {
        $user = Auth::user();
        return view('user.user.create', ['user' => $user]);
    }
    ...
    public function tree()
    {
        ...
        $user = Auth::user();
        $users = User::where('referred_by', $user->referral_code)->get()->toArray();
        foreach($users as $key => $test){
            $users[$key]['next_level'] = User::where('referred_by',$test['referral_code'])->get();
        }

        return view('user.tree', ['users' => $users, 'user' => $user]);
    }
}
